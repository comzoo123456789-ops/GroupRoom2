...
members.get('/', async (c) => {
  ...
  const result = await c.env.DB.prepare(`
    SELECT id, tenant_id, email, name, department, position, phone, role, status, avatar_color, created_at
...
members.post('/', async (c) => {
  ...
  try {
    ...
    `).bind(
      ...
      body.department || null, body.position || null, body.phone || null,
...
members.post('/bulk', async (c) => {
  ...
  const body = await c.req.json<{
    members: Array<{ name: string; email: string; phone?: string; department?: string; position?: string }>;
  }>();
  ...
  for (const m of body.members) {
    ...
    try {
      ...
      `).bind(user.tenant_id, m.email, password, m.name, m.department || null, m.position || null, m.phone || null, avatarColor).run();
      success++;
    } catch (e) {
...
members.delete('/:id', async (c) => {
  ...
  if (target.id === user.id) return c.json({ error: '본인은 삭제할 수 없습니다.' }, 400);
